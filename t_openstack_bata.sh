...
# ==============================
# ✅ 增强版：修复 crudini + 配置 Cinder 集成
# ==============================

# --- 1. 确保 crudini 正确安装（带 iniparse 依赖）---
run_step "安装 crudini 工具 (通过 pip3)" bash -c '
    yum install -y python3 python3-pip --skip-broken || { echo "❌ 安装依赖失败"; exit 1; }

    # 确保用户 bin 目录在 PATH 中
    export PATH=$PATH:/root/.local/bin

    # 安装 crudini
    pip3 install crudini --user --force-reinstall || { echo "❌ pip 安装失败"; exit 1; }

    # 验证是否能调用
    if ! crudini --version &> /dev/null; then
        echo "❌ crudini 安装后无法调用，请检查 PATH"
        ls -l /root/.local/bin/crudini || echo "文件不存在"
        exit 1
    fi

    echo "✅ crudini 安装成功: $(crudini --version)"
'
 

# --- 2. 配置 Glance：启用直接访问镜像 URL ---
run_step "配置 Glance 启用 direct URL" bash -c '
    source /root/openrc.sh
    crudini --set /etc/glance/glance-api.conf DEFAULT show_image_direct_url true
    systemctl restart openstack-glance-api
    echo "✅ Glance: show_image_direct_url = true"
'

# --- 3. 配置 Nova 与 Cinder 集成 ---
run_step "配置 Nova 与 Cinder 集成" bash -c '
    source /root/openrc.sh
    ...
    # 设置 Cinder API 信息（v3）
    crudini --set /etc/nova/nova.conf cinder catalog_info volumev3:cinderv3:publicURL
    crudini --set /etc/nova/nova.conf cinder api_insecure false

    # 增加块设备重试次数（提高稳定性）
    crudini --set /etc/nova/nova.conf DEFAULT block_device_allocate_retries 300

    # 重启相关服务
    systemctl restart openstack-nova-api openstack-nova-compute openstack-nova-scheduler

    # 验证配置写入
    if [ "$(crudini --get /etc/nova/nova.conf cinder os_region_name)" = "RegionOne" ]; then
        echo "✅ Nova-Cinder 配置写入成功"
    else
        echo "❌ Nova-Cinder 配置写入失败"
        exit 1
    fi
'

# --- 4. 验证 Cinder 服务状态 ---
run_step "验证 Cinder 服务状态" bash -c '
    source /root/openrc.sh

    # 检查 Cinder 服务是否注册
    openstack volume service list || {
        echo "⚠️ Cinder 服务未正常运行，请检查 cinder-volume 是否启动"
        systemctl status openstack-cinder-volume || true
    }

    # 检查 Nova 是否能识别 Cinder
    openstack volume type list || {
        echo "⚠️ Cinder volume type 未创建，建议手动创建"
    }
'
...
run_step "安装 Cinder 块存储服务" bash -c '
source /root/openrc.sh
source /etc/keystone/admin-openrc.sh

# --- 1. 安装 Cinder RPM 包（使用 yum，但不再包含 crudini）---
yum install -y openstack-cinder openstack-cinder-api openstack-cinder-scheduler openstack-cinder-volume lvm2 targetcli || {
    echo "❌ 安装 Cinder 包失败"
    exit 1
}

# --- 2. 创建用户 ---
useradd -s /sbin/nologin cinder 2>/dev/null || true
...
if ! vgdisplay cinder-volumes > /dev/null 2>&1; then
    if lsblk /dev/sdb > /dev/null 2>&1 && ! mount | grep /dev/sdb > /dev/null; then
        pvcreate /dev/sdb
        vgcreate cinder-volumes /dev/sdb
        echo "✅ 已创建 cinder-volumes 卷组"
    else
        echo "⚠️ 未找到可用磁盘 /dev/sdb，请手动创建卷组"
        exit 1
    fi
fi
...
glance_api_servers = http://$HOST_NAME:9292
[database]
connection = mysql+pymysql://cinder:$CINDER_DBPASS@$HOST_NAME:3306/cinder
[keystone_authtoken]
www_authenticate_uri = http://$HOST_NAME:5000/v3
auth_url = http://$HOST_NAME:5000/v3
memcached_servers = $HOST_NAME:11211
