...
# 自动获取网络配置信息
echo "正在检测网络接口..."
# 使用 mapfile 安全读取接口列表（兼容 openEuler 24）
mapfile -t INTERFACES < <(ip -o link show | awk -F': ' '$2 ~ /^(eth|ens|eno|enp|wlan)/ && $2 !~ /@/ {print $2}')
...
if [ ${#INTERFACES[@]} -eq 0 ]; then
    ...
    for i in "${!INTERFACES[@]}"; do
        iface_clean=$(echo "${INTERFACES[$i]}" | tr -d '[:space:]')
        IP_ADDR=$(ip -o -4 addr show dev "$iface_clean" 2>/dev/null | awk '{print $4}' | cut -d'/' -f1)
        echo " [$i] ${iface_clean} (${IP_ADDR:-无IP})"
    done
    while true; do
        echo -n "请选择要使用的网络接口 (0-$(( ${#INTERFACES[@]} - 1 ))): "
        read -r choice
        if [[ "$choice" =~ ^[0-9]+$ ]] && [ "$choice" -ge 0 ] && [ "$choice" -lt ${#INTERFACES[@]} ]; then
            SELECTED_INTERFACE=$(echo "${INTERFACES[$choice]}" | tr -d '[:space:]')
            break
        else
            echo "无效选择，请重新输入"
        fi
